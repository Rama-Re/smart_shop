...
def overall_similarity(product1, product2, clothes_info1=None, clothes_info2=None, shoes_info1=None, shoes_info2=None):
    ...
    if clothes_info1 and clothes_info2:
        ...
        sleeve_sim = jaccard_similarity(clothes_info1.sleeve_length, clothes_info2.sleeve_length,
...
def recommend_similar_products(target_product, all_products, clothes_info=None, shoes_info=None, top_n=5):
    similarities = []
    for product in all_products:
        sim = overall_similarity(target_product, product, clothes_info, shoes_info)
        similarities.append((product, sim))
    similarities.sort(key=lambda x: x[1], reverse=True)
    return [product for product, sim in similarities[:top_n]]


# Recommend products with lower prices
def recommend_lower_priced_products(target_product, all_products, top_n=5):
    ...
    return lower_priced_products[:top_n]
...
def recommend_based_on_text(user_description, all_products, top_n=5):
    ...
    return [product for product, sim in product_similarity_pairs[:top_n]]
...
def unified_recommendation_system(target_product, all_products, user_description=None, user_gender=None, user_age=None,
                                  clothes_info=None, shoes_info=None, top_n=5):
    similar_products = recommend_similar_products(target_product, all_products, clothes_info, shoes_info, top_n)
    lower_priced_products = recommend_lower_priced_products(target_product, all_products, top_n)
    text_similar_products = []
    if user_description:
        text_similar_products = recommend_based_on_text(user_description, all_products, top_n)
    gender_age_products = []
    if user_gender and user_age:
        gender_age_products = recommend_by_gender_and_age(user_gender, user_age, all_products, top_n)
    return {
        'similar_products': similar_products,
        'lower_priced_products': lower_priced_products,
        'text_similar_products': text_similar_products,
        'gender_age_products': gender_age_products
    }
